...
G4VPhysicalVolume *MyDetectorConstruction::Construct()
{
    ...
    G4double energy[2] = {(double)6.199209695*eV, (double)7*eV};
    G4double rindexAerogel[2] = {1.1, 1.2};
    ...
    G4ThreeVector (0., 0., 0.25*m), logicRadiator, "physRadiator", logicWorld,
    false, 0, true);
    ...
    for (G4int i = 0; i < 100; i++)
    {
        ...
        G4ThreeVector (-0.5*m+ (i+0.5) *m/100, -0.5*m+ (j+0.5) *m/100, 0.49*m), logicDetector, "physDetector", logicWorld, false, j+i*100, true);
        }
    }
